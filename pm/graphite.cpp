...
graphite_printer_t::graphite_printer_t(const std::string& prefix, int64_t timestamp) :
	path_({ prefix }),
	timestamp_(timestamp) {}

void graphite_printer_t::start_node() {}

void graphite_printer_t::end_node() {
	path_.pop_back();
}
...
void graphite_printer_t::value(int64_t v) {
	value(double(v));
}

std::string graphite_printer_t::result() const {
	return result_.str();
}
