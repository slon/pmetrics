...
TEST_F(tree_test_t, remove_empty_nodes) {
	...
	std::weak_ptr<tree_branch_t> dead_branch = root.get_branch("dead_branch")->get_branch("sub");
	dead_branch.lock()->add_leaf("dead_leaf5", make_leaf(5));

	std::shared_ptr<tree_branch_t> live_branch1 = root.get_branch("live_branch1");
	std::weak_ptr<tree_branch_t> live_branch2 = root.get_branch("live_branch2")->get_branch("sub");
